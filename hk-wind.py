...
latitude = [
    22.2889, 22.309, 22.2011, 22.2108, 22.285, 22.2142, 22.3097, 22.3119,
    22.2261, 22.4689, 22.2586, 22.2944, 22.2911, 22.3756, 22.3458, 22.4025,
    22.4361, 22.1975, 22.2931, 22.5286, 22.4753, 22.4397, 22.4714, 22.3578,
    22.3158, 22.3467, 22.3906, 22.1822, 22.4667, 22.2478
]

longitude = [
    114.1558, 113.922, 114.0267, 114.0292, 114.1128, 114.2186, 114.2133,
    114.1728, 114.1086, 113.9836, 113.9128, 114.1997, 114.0433, 114.2744,
    113.8911, 114.21, 114.0847, 114.2119, 114.1686, 114.1567, 114.2375,
    114.1839, 114.3606, 114.2178, 114.2556, 114.0864, 113.9767, 114.3033,
    114.0089, 114.1736
]
...
# ==============================================================================
# Use PingFang HK on macOS, with fallback fonts for cloud CI environments
...
title_text = (
    ...
    f"天文台此刻已知所有站點既最高10min平均風速：{abs(np.max(wind))}km/h\n"
    f"10分鐘平均風分佈圖(試驗產品）\n"
    f"強風或以上覆蓋百分比:大概{float(percent_41) * 100}%\n"
    f"烈風或以上覆蓋百分比:大概{float(percent_63) * 100}%\n"
    f"暴風或以上覆蓋百分比:大概{float(percent_88) * 100}%\n"
    f"颶風或以上覆蓋百分比：大概{float(percent_118) * 100}%\n"
    f"全圖總平均風速：{float(np.mean(wind_new))}km/h\n"
    f"Plotted by HKMETC"
)

ax.set_title(title_text, loc="left", color='k', fontweight='bold', size=16)
...
output_path = os.path.join(output_dir, "wind.png")

plt.savefig(output_path, dpi=300, bbox_inches='tight')
print(f"✅ Wind distribution plot generated and saved to {output_path}")